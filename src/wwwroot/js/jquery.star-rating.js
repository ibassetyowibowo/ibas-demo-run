...
(function ($) {
	$.fn.starRating = function (setup) {
		...
		let settings = $.extend(true, {
			...
			starIconFull: starIconFull,
			starColorEmpty: starColorEmpty,
			starColorFull: starColorFull,
			starsSize: 3, // em
			stars: 5,
			showInfo: true,
		...
		function init(wrapper) {
			...
			if (!wrapper.hasClass('js-wc-star-rating')) {
				...
				if (!settings.cursor) {
					starWrapper.css('pointer-events', 'none');
				}
				...
				for (let i = 1; i <= settings.stars; i++) {
					...
					if (settings.value == 0) {
						...
						$('<i>', {
							...
							title: settings.titles[i - 1] || i + " Sterne",
							css: {
								color: settings.starColorFull,
								margin: '2px',
								fontSize: settings.starsSize + 'em'
							},
							class: settings.starIconFull
						}).appendTo(starWrapper);
					}
				...
				if (settings.value == 0) {
					...
					if (settings.showInfo) {
						...
						$('<h4>', {
							...
							html: settings.titles[settings.value]
						}).insertBefore(starWrapper);
					}
					else {
						$('<h4>', {
